...
from losses_eval import Neg_Pearson_Loss, Gaussian_Neg_Pearson_Loss
...
for seed in seeds:
    ...
    print(f'saving results to {main_dir}')
    ...
    if use_ssl:
        ...
        elif ssl_loss_name == "Neg_Pearson":
            ssl_criterion = Gaussian_Neg_Pearson_Loss()
    ssl_loss = 0
    gradcams = None
    ...
    for epoch in range(1, num_epochs+1):
        with tqdm(training_loader, unit="batch") as tepoch:
            ...
            for inputs, labels in tepoch:
                ...
                class_losses = criterion(outputs, labels)

                # Define your class weights
                weight_0 = 272/(4932+272)
                weight_1 = 1 - weight_0

                # Generate weights based on labels
                weights = torch.where(
                    labels == 0, weight_0, weight_1)  # (batch_size,)

                # print(f'labels are: {labels} with weights: {weights}')

                # Apply the weights
                weighted_losses = class_losses * weights

                # Now average the weighted losses
                classification_loss = weighted_losses.mean()

                if use_gradcam and epoch % 5 == 0 and step == 1:
                    # GradCAM computation
                    optimizer.zero_grad()
                ...
                if use_ssl:
                    ...
                    # print(f'CAREs: {CAREs}')
                    # print(
                    #     f'CAREs max/min: {torch.max(CAREs)}/{torch.min(CAREs)}')
